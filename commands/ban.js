...
module.exports = {
    name: "ban",
    descriprion: "Bans a member from the server!",
    usage: "<member> [reason]",
    async execute(message, args) {
        ...
        if (mentionedMember) {
            ...
            if(memberPosition <= mentionedPosition) {
                return message.channel.send('**You can\'t ban this member because their role is higher/equal to yours**')
            }
            else if(botPosition <= mentionedPosition) {
                return message.channel.send('**I can\'t bann this member because their role is higher/equal to mine**')
            }
        }
        ...
        console.log(`Banned ${mentionedUser} ${reason ? `for **${reason}**` : ''}`)
    }
}