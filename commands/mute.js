...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if(!message.member.hasPermission('MANAGE_ROLES')) {
            ...
            return message.channel.send('**That is not a vaild amount of time to mute member**')
        }
        ...
        if(mentionedMember.roles.highest.position >= message.guild.me.roles.highest.position) {
            ...
            return message.channel.send('**I can\'t mute this because the `Muted` role is higher than mine**')
        }
        else if (ms(msRegex.exec(args[1])[1]) > 2592000000) {
            return message.channel.send('I can\'t mute a member for more than a month')
        }
        ...
        message.channel.send(`Muted ${mentionedMember} for **${msRegex.exec(args[1])[1]}** for ${reason ? `for **${reason}**` : ''}`)
    }
}